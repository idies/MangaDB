...
WHERE CAST(CASE WHEN (mangasampledb.nsa.sersic_mass > 0.0)
 ...
 SELECT  --q3 suetest 
  ...
  mangadatadb.ifudesign.name                                      AS "ifu.name",
  suetest.cleanspaxelprop5.emline_sew_ha_6564                  AS emline_sew_ha_6564,
...
WHERE 
 CAST(CASE WHEN (suetest.nsa.sersic_mass > 0.0)
   ...
   THEN log(suetest.nsa.sersic_mass)
      ...
      AND 
     suetest.cleanspaxelprop5.emline_sew_ha_6564 > 6.0 AND drpalias.pk = 25 AND dapalias.pk = 26;
