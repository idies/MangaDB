CREATE TABLE mangadapdb.flattabletest (
	drppipe int4 NULL,
	dappipe int4 NULL,
	plate int4 NULL,
	mangaid text NULL,
	...
	x int4 NULL,
	y int4 NULL,
	CONSTRAINT flattabletest_pkey PRIMARY KEY (pk)
)
WITH (
	OIDS=FALSE
) ;
...
CREATE TABLE mangadapdb.flat_ssd tablespace manga_ssd AS SELECT * fom mangadapdb.flattabletest;
create
    index flat_binid_ix on
    flattabletest
        using btree(binid) with(
        fillfactor = '100'
    ) ;
create
    index flat_dappipe_ix on
    flattabletest
        using btree(dappipe) with(
        fillfactor = '100'
    ) ;
create
    index flat_drppipe_ix on
    flattabletest
        using btree(drppipe) with(
        fillfactor = '100'
    ) ;
create
    index flat_emline_gflux_ha_6564_ix on
    flattabletest
        using btree(emline_gflux_ha_6564) with(
        fillfactor = '100'
    ) ;
create
    index flat_emline_sew_ha_6564_ix on
    flattabletest
        using btree(emline_sew_ha_6564) with(
        fillfactor = '100'
    ) ;
create
    index flat_name_ix on
    flattabletest
        using btree(name) with(
        fillfactor = '100'
    ) ;
create
    index flat_nsa_pk_ix on
    flattabletest
        using btree(nsa_pk) with(
        fillfactor = '100'
    ) ;
create
    index flat_plate_ix on
    flattabletest
        using btree(plate) with(
        fillfactor = '100'
    ) ;
create
    index flat_plateifu_ix on
    flattabletest
        using btree(plateifu) with(
        fillfactor = '100'
    ) ;
